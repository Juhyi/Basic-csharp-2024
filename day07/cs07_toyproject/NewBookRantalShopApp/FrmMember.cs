...
namespace NewBookRantalShopApp
{
    public partial class FrmMember : MetroForm
    {
        ...
        private void BtnNew_Click(object sender, EventArgs e)
        {
            ...
            TxtMemberIdx.Focus(); // 사용자번호는 자동증가 -> 입력불가
            CboLevels.SelectedIndex = -1;
        ...
        private void BtnDel_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection conn = new SqlConnection(Helper.Common.ConnString))
            {
                ...
                SqlCommand cmd = new SqlCommand(query, conn);
                SqlParameter prmBookIdx = new SqlParameter("@bookidx", TxtMemberIdx.Text);
        ...
        private void RefreshData()
        {
            ...
            using (SqlConnection conn = new SqlConnection(Helper.Common.ConnString))
            {
                ...
                var query = @"SELECT [memberIdx]
                                      ...
                                      ,[Mobile]
                                      ,[Email]
                                  FROM [membertbl];"; // 화면에 필요한 테이블쿼리로 변경
