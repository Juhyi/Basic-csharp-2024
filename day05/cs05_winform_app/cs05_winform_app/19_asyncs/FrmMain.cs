...
namespace _19_asyncs
{
    public partial class FrmMain : Form
    {
        ...
        long CopySync(string srcPath, string destPath)
        {
            ...
            return totalCopied; // 복사할 파일사이즈 리턴
        }

        // 비동기처리시 asynx, await 키워드가 가장 중요
        // asynx : 나는 비동기메서드야 정의
        // await : 비동기메서드가 끝날때까지 기다릴게 정의
        // 비동기를 처리하는 메서드명이 ...Aync()로 끝남
        // asynx는 메서드 리턴값에 작성, 리턴값은 Task<리턴값>
        ...
        async Task<long> CopyAsync(string srcPath, string destPath)
        {
            ...
            {   
                using (FileStream toStream = new FileStream(destPath, FileMode.Create))
                {   
                    byte[] buffer = new byte[1024 *1024]; // Test시 10으로 변경해서 실행
                    int nRead = 0;
