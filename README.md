...
- TIP, C# 주석 중 영역을 지정할 수 있는 주석
    ...
    ![region주석](https://raw.githubusercontent.com/juhyi/basic-csharp-2024/main/images/cs002.png)

## 3일차
- 람다식 
    - 익명 메서드를 만드는 방식 중에 하나 - delegate, lambda expression
    - 익명 메서드시 코딩량을 줄여줌, 프로퍼티 사용시에도 코딩양이 줄어듬
    - 익명 메서드 사용시 마다 대리자를 선언해야하기 때문에
        - Func, Action을 MS에서 미리 만들어둠.

- LINQ(Language Integrated Query)
    - c#에서 통합된 데이터 질의 기능(DB SQL과 거의 동일)
    - group by에 집계함수가 아닌 것 외에는 SQL과 거의 동일
    - 단, 키워드 사용순서가 다른 것을 인지해야
    - LINQ만 고집하면 안됨. 기존의 C# 로직을 사용해야 할 수도...
...
- 파이썬 실행
    ...
    - 해당 프로그램 종속성, 마우스 오른쪽 버튼 > NugetPackage 관리
        1. Python 엔진,scope,설정경로 생성
...
- 윈폼 UI 개발(계속)
    ...
    - 파일 입출력 추가
        ...
        <img src="https://raw.githubusercontent.com/juhyi/basic-csharp-2024/main/images/cs003.png" width="850">
    ...
    - 비동기 작업 앱
        ...
        ![비동기앱](https://raw.githubusercontent.com/juhyi/basic-csharp-2024/main/images/cs004.png)

## 6일차
- 토이 프로젝트
    - 윈도우 탐색기 앱(컨트롤 학습)
        - MyExplorer 프로젝트 생성
        - 아이콘검색, png 2 ico 구글링 웹 사이트
        - 트리뷰, 리스트뷰 기능 추가

        ![중간결과](https://raw.githubusercontent.com/juhyi/basic-csharp-2024/main/images/cs005.png)
...
- 토이 프로젝트        
    ...
    ```cs
    // 값 형식 변수에 null값을 넣을 수 있도록 만들어준 기능 Nullable, 변수명 뒤에 ?만 추가할 것!
    int? a = null;
    double? b = null;
    float? c = null;
    ```
...
## 9일차
- 토이 프로잭트
    - 기존에 만들어진 폼을 복사해서 변경할 시 
        - ***.cs클래스 명, 생성자, *.Designer.cs에 있는 클래스명** 3군데 이름변경
    - 도서관리 앱
        - 공동 클래스...
        - 책 장르 관리
        - 책정보 관리
...
- 토이 프로젝트
    - 도서관리 앱
        ...
        ![책 대여 프로그램](https://raw.githubusercontent.com/juhyi/basic-csharp-2024/main/images/cs006.png)
...
- 학점 관리 앱
    - 로그인 화면 구현하기
        - 학번, 패스워드로 로그인
            - SQL Server에서 usertbl 테이블 생성하기.
        - 패스워드 암호화 구현
        - 패스워드 보기누르면 패스워드 확인 가능
    ...
    - 메뉴화면 기능
        - 메뉴 누르면 각 화면 띄우기
        - 로그아웃 버튼 클릭시 다시 로그인화면으로 되돌아감
        
    - 특징
        - 학생정보 프로그램에 학점 계산기 기능 추가함.
        - db와 연결해서 개인 성적 관리 가능.


    - 배운점
        - sql Server database의 value을 C#에 가져올때는 파라미터로 가지고 오기 때문에 @ 기호 필요함.
...
https://github.com/Juhyi/Basic-csharp-2024/assets/158007405/1d41969e-2fff-4215-8d2d-b6b9e86b3297
